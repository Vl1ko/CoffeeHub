...
import ProductList from '@/components/ProductListPrachSW.vue';
...
onMounted(async () => {
    try {
        const {data} = await axios.get('https://ff3b1eb790ba669b.mokky.dev/prachSW')
        items.value = data
    } catch (error) {
...
<template>
    <div class="max-w-100% overflowx-hidden">
        ...
        <h2 class="text-4xl font-semibold text-center m-10">Прачечный, 3<br>Десерты Sweet Marin</h2>
