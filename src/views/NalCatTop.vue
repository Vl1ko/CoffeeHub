...
import ProductList from '@/components/ProductListNalTop.vue';
...
onMounted(async () => {
    try {
        const {data} = await axios.get('https://c926c1b080b9a593.mokky.dev/nalTop')
        items.value = data
    } catch (error) {
...
<template>
    <div class="max-w-100% overflowx-hidden">
        ...
        <h2 class="text-4xl font-semibold text-center m-10">Наличная, 49<br>Топпинги к напиткам</h2>
