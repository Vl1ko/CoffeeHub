...
import ProductList from '@/components/ProductListSadTop.vue';
...
onMounted(async () => {
    try {
        const {data} = await axios.get('https://77166a58ad9d5215.mokky.dev/sadTop')
        items.value = data
    } catch (error) {
...
<template>
    <div class="max-w-100% overflowx-hidden">
        ...
        <h2 class="text-4xl font-semibold text-center m-10">Садовая, 89<br>Топпинги к напиткам</h2>
