...
import ProductList from '@/components/ProductListBolshProch.vue';
...
onMounted(async () => {
    try {
        const {data} = await axios.get('https://27952d008f4c78ac.mokky.dev/bolshProch')
        items.value = data
    } catch (error) {
...
<template>
    <div class="max-w-100% overflowx-hidden">
        ...
        <h2 class="text-4xl font-semibold text-center m-10">Большевиков, 9<br>Прочее</h2>
